...
NewtonRaphson<-function(func,StartingValue,Tolerance,MaxNumberOfIterations){
  ...
  while ((i<MaxNumberOfIterations)&&(Deviation>Tolerance))
  {
    ...
    #  The functions F1...F2 each return two values, so you need to do something like this...
    Z<-func(x)
    # PM: To be safe, check that the function and it's derivative are defined at X (either could be NaN if you are unlucky)
    if ((Z[1]=="NaN")||(Z[2]=="NaN")){
    ...
    # You need a bit of code in here to findse the new value of x using Newton-Raphon's formula
    ####  ADD CODE HERE  ####
    #### I
